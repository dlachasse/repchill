...
function updateAttendancePHP($player,$date,$attending,$sub=0,$echo=True){
	//init vars
	$returnMsg='';

	//only process is subs have names >2 letters
	if(strlen($player)<1){
		echo 'ok, player is less than 1';
		break;
	}

	//check if person is already attending this game.
	$alreadyAttending = False;
	$query = "SELECT name FROM rollcall WHERE `game`=FROM_UNIXTIME($date) AND `name`=\"$player\"";
	...
	if($attending && !$alreadyAttending){
		...
		$query = "INSERT INTO rollcall (name,game,sub) VALUES (\"$player\",FROM_UNIXTIME($date),\"$sub\")";
		$result = mysql_query($query) or die(mysql_error());
	...
	//not attending!
		...
		if($alreadyAttending){
			$query = "DELETE FROM rollcall WHERE `game`=FROM_UNIXTIME($date) AND `name`='".$player."'";
			$result = mysql_query($query) or die(mysql_error());
	...
	$returnMsg="You, kind sir or miss: $player<br/>
				...
				Game Date: ".date('l',$date)." the effin ".date('jS, F Y h:i A',$date)."<br/><br/>
				\n".$returnMsg;
	if($echo){
		echo trim($returnMsg);	
	}
...
$nextGameDate = mysql_real_escape_string($_POST['nextGameDate']);
$sub = mysql_real_escape_string($_POST['sub']);
$echo = mysql_real_escape_string($_POST['echo']);

updateAttendancePHP($player,$nextGameDate,$attending,$sub,$echo);
